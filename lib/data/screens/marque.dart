...
class Pub {
  ...
  factory Pub.fromJson(Map<String, dynamic> json) {
    return Pub(
      ...
      statut: json['statut'] ?? '',
      duree: json['duree'] ?? '',
      dateDemande: DateTime.tryParse(json['dateDemande'] ?? '') ?? DateTime.now(),
...
class _MarqueState extends State<Marque> with SingleTickerProviderStateMixin {
  ...
  bool _isPubValid(Pub pub) {
    ...
    if (pub.dateFin != null) {
      ...
      _logger.info('[DEBUG] 🔍 Pub ${pub.id} avec dateFin: ${pub.dateFin}, maintenant: $now, valide: $isValid');
      return isValid;
    }
    
    final dateDebut = pub.dateDebut ?? pub.dateDemande;
    _logger.info('[DEBUG] 🔍 Pub ${pub.id} sans dateFin, utilise dateDebut: $dateDebut');
    ...
    _logger.info('[DEBUG] 🔍 Pub ${pub.id} calculée: dateDebut=$dateDebut, durée=${pub.duree} ($dureeJours jours), dateFin=$dateFin, valide=$isValid');
    return isValid;
  }
  ...
  Future<void> fetchArticlesPieces() async {
    ...
    try {
      ...
      String url = getBaseUrl() + '/articles?type=piece&statut=en_ligne&vendu=false';
      ...
      if (response.statusCode == 200) {
        ...
        try {
          ...
          final piecesFiltered = allPieces.where((p) => (p.statut ?? 'en_ligne') != 'vendu').toList();
          
          _logger.info('[DEBUG] Total pièces reçues: ${allPieces.length}');
          _logger.info('[DEBUG] Pièces après filtrage vendu: ${piecesFiltered.length}');
          
          // Log des statuts pour déboguer
          for (var p in allPieces) {
            if ((p.statut ?? 'en_ligne') == 'vendu') {
              _logger.info('[DEBUG] Pièce vendue trouvée: ${p.title} - Statut: ${p.statut}');
            }
          }
  ...
  Future<void> fetchVoituresRecommandees() async {
    ...
    try {
      final user = FirebaseAuth.instance.currentUser;
      final idToken = await user?.getIdToken();
      String url = getBaseUrl() + '/articles?type=voiture&statut=en_ligne&vendu=false';
      ...
      if (response.statusCode == 200) {
        ...
        if (!mounted) return;
        final allVoitures = data.map((e) => ArticleVoiture.fromJson(e)).toList();
        final voituresFiltered = allVoitures.where((v) => (v.statut ?? 'en_ligne') != 'vendu').toList();
        
        _logger.info('[DEBUG] Total voitures reçues: ${allVoitures.length}');
        _logger.info('[DEBUG] Voitures après filtrage vendu: ${voituresFiltered.length}');
        
        // Log des statuts pour déboguer
        for (var v in allVoitures) {
          if ((v.statut ?? 'en_ligne') == 'vendu') {
            _logger.info('[DEBUG] Voiture vendue trouvée: ${v.titre} - Statut: ${v.statut}');
          }
        }
  ...
  Future<void> fetchPubs() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (!mounted) return;
        setState(() {
          pubsALaUne = pubs.where((p) => p.typePub == 'À la une' && _isPubValid(p)).toList();
          _logger.info('[DEBUG] 📺 Total pubs reçues: ${pubs.length}');
          _logger.info('[DEBUG] 📺 Publicités À la une: ${pubsALaUne.length} valides sur ${pubs.where((p) => p.typePub == 'À la une').length} totales');
          for (var pub in pubs.where((p) => p.typePub == 'À la une')) {
            final isValid = _isPubValid(pub);
            _logger.info('[DEBUG] 📺 Pub ${pub.id}: statut=${pub.statut}, dateDebut=${pub.dateDebut}, dateFin=${pub.dateFin}, valide=$isValid');
            if (!isValid && pub.statut == 'valide') {
              _logger.info('[DEBUG] 📺 Pub ${pub.id} rejetée pour expiration - maintenant: ${DateTime.now()}, dateFin: ${pub.dateFin}');
            }
          }
          isLoadingPubs = false;
  ...
  Future<void> fetchPubsSponsorisees() async {
    ...
    try {
      final user = FirebaseAuth.instance.currentUser;
      final idToken = await user?.getIdToken();
      String url = getBaseUrl() + '/publicites?typePub=Sponsorisée&statut=valide';
      final response = await http.get(
        Uri.parse(url),
        headers: {
          'Authorization': 'Bearer $idToken',
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          _logger.info('[DEBUG] ⭐ Total pubs sponsorisées reçues: ${pubs.length}');
          _logger.info('[DEBUG] ⭐ Publicités Sponsorisées: ${pubsSponsorisees.length} valides sur ${pubs.length} totales');
          for (var pub in pubs) {
            final isValid = _isPubValid(pub);
            _logger.info('[DEBUG] ⭐ Pub ${pub.id}: statut=${pub.statut}, dateDebut=${pub.dateDebut}, dateFin=${pub.dateFin}, valide=$isValid');
            if (!isValid && pub.statut == 'valide') {
  ...
  Widget buildVoituresRecommandeesGrid() {
    ...
    final voituresEnLigne =
        voituresRecommandees
            .where((v) => (v.statut ?? 'en_ligne') == 'en_ligne' && (v.statut ?? '') != 'vendu')
            .toList();
                  ...
                  Positioned(
                    ...
                    child: GestureDetector(
                      ...
                      child: CircleAvatar(
                        ...
                        child: Icon(
                          _favoriteArticleIds.contains(voiture.id.toString())
                              ? Icons.favorite
                      ...
                      decoration: BoxDecoration(
                        color:
                            (voiture.condition?.toLowerCase() == 'nouveau' ||
                                    voiture.condition?.toLowerCase() == 'neuf')
                                ? Colors.purple
  ...
  Widget buildPiecesGrid() {
    ...
    final piecesEnLigne =
        articlesPieces
            .where((p) => (p.statut ?? 'en_ligne') == 'en_ligne' && (p.statut ?? '') != 'vendu')
            .toList();
    // Filtrage recherche
                          ...
                          children: [
                            ...
                            Positioned(
                              ...
                              child: GestureDetector(
                                onTap:
                                    () =>
                                        _toggleFavorite(voiture.id.toString()),
                                child: CircleAvatar(
                                  radius: 16,
                                  backgroundColor: Colors.white,
                                  child: Icon(
    ...
    return Column(
      ...
      children: [
        ...
        if (isLoadingPubs)
          ...
          SizedBox(
            height: 100,
            child: Center(child: Text(errorPubs!)),
          )
